...
const KpiCard = ({
  ...
  return (
    <div
      ...
      className={`bg-white border border-slate-200 shadow-sm p-4 flex flex-col justify-between h-full min-h-[130px] transition-all select-none ${
        isClickable
          ? 'cursor-pointer hover:shadow-md hover:-translate-y-0.5 hover:border-slate-300 group'
          : 'hover:border-slate-300'
      } ${c.border} ${isLoading ? 'animate-pulse' : ''}`}
    >
      {/* Top Header: Icon on Left, Badges or Trend on Right */}
      <div className="flex items-start justify-between gap-2 mb-2">
        <span className={`flex h-10 w-10 shrink-0 items-center justify-center transition-transform ${isClickable ? 'group-hover:scale-105' : ''} ${c.bg}`}>
          {Icon && <Icon size={19} className={c.icon} aria-hidden="true" />}
        </span>

        {/* Right Header: Badges or Trend Pill or Click Arrow */}
        <div className="flex items-center gap-1.5 justify-end">
          ...
          {isClickable && (
            <ArrowUpRight
              size={15}
              className="text-slate-300 group-hover:text-slate-600 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all shrink-0 ml-1"
            />
          )}
        </div>
      </div>
      ...
      <div className="mt-auto">
        ...
        <p className="text-xs font-semibold text-slate-600 mt-0.5 truncate group-hover:text-slate-900 transition-colors">{title}</p>
